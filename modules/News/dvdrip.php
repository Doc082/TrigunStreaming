...
function theindex($catid) {
	...
	if ($multilingual == 1) {
		$querylang = "(alanguage='$currentlang' OR alanguage='')"; /* the OR is needed to display stories who are posted to ALL languages */
	} else {
		$querylang = "";
	}
	include("header.php");
...
div.pagination span.current {
	...
	$rows = 0;
	$qry="dvd-rip";
	$query = "SELECT COUNT(*) as num FROM ".$prefix."_stories where $querylang AND (s.title LIKE '%$qry2%' OR s.hometext LIKE '%$qry3%' OR s.bodytext LIKE '%$qry3%' OR s.notes LIKE '%$qry3%') ORDER BY sid DESC";
...
$news_sel = "SELECT sid, aid, title, time, hometext, bodytext, comments, counter, topic, informant, notes, acomm, score, ratings FROM ".$prefix."_stories where $querylang AND (s.title LIKE '%$qry2%' OR s.hometext LIKE '%$qry3%' OR s.bodytext LIKE '%$qry3%' OR s.notes LIKE '%$qry3%') ORDER BY sid DESC LIMIT $start, $limit";
