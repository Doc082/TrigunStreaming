...
function adminMain() {
	...
	if (is_active("News")) {
		...
		$query = "SELECT COUNT(*) as num FROM ".$prefix."_stories $querylang ORDER BY sid DESC";
	    $total_pages = $db->sql_fetchrow($db->sql_query($query));
	    $total_pages = $total_pages[num];
		$lastpage = ceil($total_pages/20);
		...
		$result6 = $db->sql_query("SELECT sid, aid, title, time, topic, informant, alanguage FROM ".$prefix."_stories $queryalang ORDER BY sid DESC LIMIT $pstart, $pstop");
		...
		if($lastpage>1){
			if($pag>1) echo "<a href='admin.php?pag=$prev'><< Precedente</a> - ";
			for($counter=1; $counter<($lastpage+1); $counter++){
				if($counter==$pag) echo"<a href='admin.php?pag=$counter'><strong>$counter</strong></a>";
				else echo"<a href='admin.php?pag=$counter'>$counter</a>";	
				if($counter<$lastpage) echo " - ";
			}
			if($pag<$lastpage)echo " - <a href='admin.php?pag=$next'>Successiva >></a>";
			echo"\n";
		}
		echo "</center>";
		CloseTable();
